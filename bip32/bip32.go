...
func (key *Key) NewChildKey(childIdx uint32) (*Key, error) {
	...
	if key.Type == KeyTypeEd25519 {
		...
		k := append([]byte{0x0}, key.Key...)
		data := append(k, iBytes...)

		hasher := hmac.New(sha512.New, key.ChainCode)
		...
		childKey.ChainCode = intermediary[32:]
...
func (key *Key) getIntermediary(childIdx uint32) ([]byte, error) {
	...
	hmacer := hmac.New(sha512.New, key.ChainCode)
	_, err := hmacer.Write(data)
	if err != nil {
		return nil, err
	}
	return hmacer.Sum(nil), nil
}
